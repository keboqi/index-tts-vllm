...
const CHUNK_SPLIT_MIN_SILENCE_MS = Number(document.querySelector('meta[name="chunk-split-min-silence-ms"]')?.content || 1000);
        ...
        const ENDPOINTS = Object.freeze({
            ...
            HIGGS_ENHANCE_TEXT: '/api/higgs/enhance_text',
        ...
        function buildHiggsEnhancePrompt(sourceText) {
            const text = String(sourceText || '').trim();
            return `You are a text director for BosonAI Higgs TTS 3.
...
Enhanced Higgs TTS input:`;
        ...
        function getHiggsSourceText() {
            const textInput = document.getElementById('text');
            return textInput ? textInput.value.trim() : '';
        }

        function updateHiggsEnhancePromptPreview() {
            const sourceText = getHiggsSourceText();
            const promptEl = document.getElementById('higgsEnhancePrompt');
            const copyBtn = document.getElementById('copyHiggsEnhancePromptBtn');
            const generateBtn = document.getElementById('generateHiggsEnhancedBtn');
            const useBtn = document.getElementById('useHiggsEnhancedBtn');
            const enhancedEl = document.getElementById('higgsEnhancedText');
            if (promptEl) {
                promptEl.value = sourceText ? buildHiggsEnhancePrompt(sourceText) : '';
            }
            if (copyBtn) copyBtn.disabled = !sourceText;
            if (generateBtn) generateBtn.disabled = !sourceText;
            if (useBtn) useBtn.disabled = !(enhancedEl && enhancedEl.value.trim());
        }
        ...
        async function copyHiggsEnhancePrompt() {
            const sourceText = getHiggsSourceText();
            if (!sourceText) {
                showStatus('Enter text before copying the Higgs prompt.', 'error', 'higgsEnhanceStatus');
                return;
            }
            const prompt = buildHiggsEnhancePrompt(sourceText);
            const promptEl = document.getElementById('higgsEnhancePrompt');
            if (promptEl) promptEl.value = prompt;
            try {
                await copyTextToClipboard(prompt);
                showStatus('Full Higgs LLM prompt copied.', 'success', 'higgsEnhanceStatus');
            } catch (error) {
                showStatus(`Copy failed: ${error.message}`, 'error', 'higgsEnhanceStatus');
            }
        }

        async function generateHiggsEnhancedText() {
            const sourceText = getHiggsSourceText();
            const generateBtn = document.getElementById('generateHiggsEnhancedBtn');
            const enhancedEl = document.getElementById('higgsEnhancedText');
            const promptEl = document.getElementById('higgsEnhancePrompt');
            const modelEl = document.getElementById('higgsEnhanceGeminiModel');
            const apiKeyEl = document.getElementById('higgsEnhanceGeminiApiKey');
            if (!sourceText) {
                showStatus('Enter text before generating Higgs enhancement.', 'error', 'higgsEnhanceStatus');
                return;
            }
            const payload = {
                text: sourceText,
                gemini_model: modelEl ? modelEl.value : ''
            };
            ...
            showStatus('Generating enhanced Higgs text...', 'info', 'higgsEnhanceStatus');
            try {
                const response = await fetch(ENDPOINTS.HIGGS_ENHANCE_TEXT, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(payload)
                });
                if (!response.ok) {
                    throw new Error(await parseHttpError(response, `Higgs enhancement failed (${response.status})`));
                }
                const data = await response.json();
                if (promptEl && data.prompt) promptEl.value = data.prompt;
                if (enhancedEl) enhancedEl.value = data.enhanced_text || '';
                updateHiggsEnhancePromptPreview();
                showStatus('Enhanced Higgs text generated.', 'success', 'higgsEnhanceStatus');
            } catch (error) {
                showStatus(`Higgs enhancement error: ${error.message}`, 'error', 'higgsEnhanceStatus');
            } finally {
                if (generateBtn) {
                    generateBtn.disabled = !getHiggsSourceText();
                    generateBtn.textContent = 'Generate enhanced text';
                }
            }
        }

        function useHiggsEnhancedText() {
            const textInput = document.getElementById('text');
            const enhancedEl = document.getElementById('higgsEnhancedText');
            const enhancedText = enhancedEl ? enhancedEl.value.trim() : '';
            if (!textInput || !enhancedText) {
                showStatus('No enhanced Higgs text to use.', 'error', 'higgsEnhanceStatus');
                return;
            }
            textInput.value = enhancedText;
            updateHiggsEnhancePromptPreview();
            showStatus('Enhanced Higgs text loaded into the synthesis text box.', 'success', 'higgsEnhanceStatus');
        }

        let higgsEnhancerExpanded = false;

        function toggleHiggsEnhancer() {
            higgsEnhancerExpanded = !higgsEnhancerExpanded;
            updateHiggsEnhancerVisibility();
        }

        function updateHiggsEnhancerVisibility() {
            const panel = document.getElementById('higgsEnhancerPanel');
            const toggleRow = document.getElementById('higgsEnhancerToggleRow');
            const toggleBtn = document.getElementById('toggleHiggsEnhancerBtn');
            const backendEl = document.getElementById('ttsBackend');
            const isHiggs = Boolean(backendEl && backendEl.value === 'higgs');
            if (!isHiggs) {
                higgsEnhancerExpanded = false;
            }
            if (toggleRow) {
                toggleRow.style.display = isHiggs ? 'flex' : 'none';
            }
            if (panel) {
                panel.style.display = isHiggs && higgsEnhancerExpanded ? 'block' : 'none';
            }
            if (toggleBtn) {
                toggleBtn.textContent = higgsEnhancerExpanded ? 'Hide Higgs enhancer' : 'Show Higgs enhancer';
                toggleBtn.setAttribute('aria-expanded', String(isHiggs && higgsEnhancerExpanded));
            }
            updateHiggsEnhancePromptPreview();
        }
