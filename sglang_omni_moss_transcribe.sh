...
# SGLang-Omni MOSS Transcribe+Diarize Docker manager
# Usage:
#   bash sglang_omni_moss_transcribe.sh deploy
#   bash sglang_omni_moss_transcribe.sh start
#   bash sglang_omni_moss_transcribe.sh stop
#   bash sglang_omni_moss_transcribe.sh status
#   bash sglang_omni_moss_transcribe.sh logs
#   bash sglang_omni_moss_transcribe.sh test input.wav
#
# Optional env overrides:
#   NAME=index-tts-moss-transcribe-sglang
...
#   PROJECT_ROOT=$HOME/index-tts-moss-transcribe-sglang-workspace
#   HF_CACHE=$HOME/.cache/huggingface
#   EXTRA_ARGS="--some-sgl-omni-arg value"

NAME="${NAME:-index-tts-moss-transcribe-sglang}"
IMAGE="${IMAGE:-lmsysorg/sglang-omni:dev}"
MODEL="${MODEL:-OpenMOSS-Team/MOSS-Transcribe-Diarize}"
PORT="${PORT:-8003}"
...
PROJECT_ROOT="${PROJECT_ROOT:-$HOME/index-tts-moss-transcribe-sglang-workspace}"
...
PID_FILE_IN="/workspace/moss_transcribe_${PORT}.pid"
PGID_FILE_IN="/workspace/moss_transcribe_${PORT}.pgid"
LOG_FILE_IN="/workspace/logs/moss_transcribe_${PORT}.log"
LIFECYCLE_LOCK_FILE="$PROJECT_ROOT/.sglang_omni_moss_transcribe_${PORT}.lifecycle.lock"
...
usage() {
  cat <<USAGE
SGLang-Omni MOSS Transcribe+Diarize manager

Commands:
  deploy          Pull Docker image, create persistent container, clone/install sglang-omni, download model
  start           Start the MOSS transcription server inside the container
  stop            Stop only the MOSS transcription server process, keep container alive
  restart         Stop then start the MOSS transcription server
  status          Show container/server status and URLs
  logs            Tail server logs; set LINES=500 for more history
  shell           Open an interactive shell inside the container
  test [file]     Send audio to /v1/audio/transcriptions with verbose_json
...
Examples:
  ...
  MEM_FRACTION_STATIC=0.20 bash $0 start
  MAX_RUNNING_REQUESTS=8 CUDA_GRAPH_MAX_BS=8 bash $0 start
USAGE
}
...
if [ ! -x .venv/bin/python ]; then
  echo "Creating Python 3.12 uv venv..."
  uv venv .venv -p 3.12
else
...
start_server() {
  ...
  if command -v curl >/dev/null 2>&1 && curl -fsS --max-time 3 "http://localhost:${PORT}/v1/models" >/dev/null 2>&1; then
    echo "MOSS transcription server is already healthy on port ${PORT}; reusing it."
    return 0
  fi

  if server_running_in_container; then
    echo "MOSS transcription server is already running."
    status
    return 0
  fi

  echo "Starting MOSS transcription server on port ${PORT}..."
  docker exec -i \
    -e MODEL="$MODEL" \
    -e PORT="$PORT" \
...
if ! kill -0 "$PID" 2>/dev/null; then
  rm -f "$PID_FILE_IN"
  rm -f "$PGID_FILE_IN"
  echo "MOSS transcription server failed to start. Recent log output:" >&2
...
stop_server() {
  ...
  echo "Stopping MOSS transcription server if running..."
  docker exec -i \
    -e MODEL="$MODEL" \
    -e REPO_IN="$REPO_IN" \
    -e PID_FILE_IN="$PID_FILE_IN" \
...
if [ "$STOPPED" = "1" ]; then
  ...
  echo "No matching MOSS transcription server process found."
fi
'
}
...
test_request() {
  ...
  if [[ -z "$audio" || ! -f "$audio" ]]; then
    echo "Usage: bash $0 test path/to/audio.wav" >&2
    exit 2
  fi
  echo "Sending transcription request to http://localhost:${PORT}/v1/audio/transcriptions"
