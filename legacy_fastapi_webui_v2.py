...
TRANSLATION_TTS_CONCURRENCY = min(
...
def _ensure_runtime_directories() -> None:
    """Create mutable application directories during lifespan startup."""
    directories = (
        ROOT_OUTPUT_DIR,
        ROOT_OUTPUT_DIR / "tasks",
        (APP_DIR / "prompts").resolve(),
        (APP_DIR / "speaker_presets").resolve(),
        (APP_DIR / SPEAKER_REFERENCE_DIR).resolve(),
        Path(TRANSLATE_OUTPUT_DIR),
        Path(TRANSLATE_SESSION_MEDIA_DIR),
        Path(VIDEO_DOWNLOAD_DIR),
        Path(VIDEO_AUDIO_CACHE_DIR),
        Path(VIDEO_SNAPSHOT_CACHE_DIR),
        Path(COOKIES_DIR),
        Path(CLEARVOICE_CACHE_DIR),
        Path(AUDIO_SEPARATOR_CACHE_DIR),
        Path(AUDIO_SEPARATOR_MODEL_DIR),
        Path(GEMINI_CACHE_DIR),
        Path(SPLIT_AUDIO_CACHE_DIR),
    )
...
def _streaming_audio_frame(chunk_idx: int, audio_bytes: bytes, is_last: bool) -> bytes:
    return audio_chunk_frame(chunk_idx, audio_bytes, is_last=is_last)


def _streaming_keepalive_frame(message: str, *, elapsed_seconds: Optional[int] = None) -> bytes:
    return keepalive_frame(message, elapsed_seconds=elapsed_seconds)
...
def _parse_srt_timestamp_to_ms(timestamp: str) -> int:
    return parse_subtitle_timestamp(timestamp)


def _parse_srt_file_with_timestamps(srt_content: str) -> List[Dict[str, Any]]:
    return parse_subtitle_entries(srt_content)
...
async def _synthesize_tts_to_file(
    *,
    ...
    request = SynthesisRequest(
        ...
        speaker_preset=speaker_preset,
        prompt_audio=spk_audio_prompt or None,
        reference_text=reference_text,
        language=language,
        ...
        emotion_text=emo_text if use_emo_text else None,
    ...
    result_path = await TTS_BACKEND_REGISTRY.get(backend).synthesize(request)
